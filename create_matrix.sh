...
cat /tmp/tunnel-$$.tmp | awk -v ASN=${ASN} 'BEGIN {numlinks=0; linksfile=sprintf("as%s-links.txt", ASN); while((getline line <linksfile) > 0) {n=split(line, fields); if(n==2) {print $0; split(fields[1], peers, ":"); link[numlinks++]=sprintf("%s:%s", peers[1], peers[2]); link[numlinks++]=sprintf("%s:%s", peers[2], peers[1]);}} /* printf("# %d links read from %s\n", numlinks, linksfile); */} {lnk=$1; tpe=$2; found=0; for(i=0; i<numlinks && found==0; i++) {if(lnk==link[i]) {found=1; /* printf("%s matches %s\n", lnk, link[i]); */}} if(found==0) {printf("%s\n", $0);}}' | sort -u >as${ASN}-tunnel.txt
rm /tmp/tunnel-$$.tmp
