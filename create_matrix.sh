...
awk < as${ASN}-hosts.txt >/tmp/tunnel-$$.tmp '{host[++num]=$1;} END {for(i=1; i<=num; i++) { for(j=i; j<=num; j++) {if(host[i] != host[j]) {tt="l2tp"; if(substr(host[i], 1, 2)=="gw" && substr(host[j], 1, 2)=="gw") tt="l2tp-eth"; printf("%s:%s %s\n", host[i], host[j], tt);}}}}'
